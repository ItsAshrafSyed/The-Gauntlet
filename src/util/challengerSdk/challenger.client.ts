...
export class ChallengerClient {
	...
	setChallengerProgram(idl?: Idl, programId?: PublicKey) {
		...
		if (idl && programId) {
			...
			this.challengerProgram = anchor.workspace
				.BountyPool as Program<Challenger>;
		}
	}
	...
	async fetchAllCruxPDAs(cruxManager?: PublicKey) {
		...
		if (cruxManager) {
			console.log(
				"Found a total of",
				pdas.length,
				"crux PDAs for crux manager with address",
				cruxManager.toBase58()
			);
		} else {
	...
	async fetchAllUserProfilePDAs(crux?: PublicKey) {
		...
		if (crux) {
			console.log(
				"Found a total of",
				pdas.length,
				"user profile PDAs for crux with address",
				crux.toBase58()
			);
		} else {
	...
	async fetchAllUserProfilePDAsByProfileOwner(profileOwner: PublicKey) {
		...
		console.log(
			"Found a total of",
			pdas.length,
			"user profile PDAs for profile owner with address",
			profileOwner.toBase58()
		);
		return pdas;
	}
	...
	async fetchAllChallengePDAs(crux?: PublicKey) {
		...
		if (crux) {
			console.log(
				"Found a total of",
				pdas.length,
				"challenge PDAs for crux with address",
				crux.toBase58()
			);
		} else {
	...
	async fetchAllSubmissionPDAs(challenge?: PublicKey) {
		...
		if (challenge) {
			console.log(
				"Found a total of",
				pdas.length,
				"submission PDAs for challenge account with address",
				challenge.toBase58()
			);
		} else {
	...
	async fetchAllSubmissionPDAsByUserProfile(userProfile: PublicKey) {
		...
		console.log(
			"Found a total of",
			pdas.length,
			"submission PDAs for user profile with address",
	...
	async initCrux(
		...
		// Derive PDAs
		const [cruxAuthority, cruxAuthBump] = await findCruxAuthorityPDA(
			crux.publicKey
		);
		const [cruxTreasury, cruxTreasuryBump] = await findCruxTreasuryPDA(
			crux.publicKey
		);
		...
		console.log(
			"initializing crux account with pubkey: ",
			crux.publicKey.toBase58()
		);

		// Transaction
		const txSig = await this.challengerProgram.methods
			.initCrux(cruxAuthBump, cruxFees)
			.accounts({
				crux: crux.publicKey,
				cruxManager: isKp(cruxManager)
					? (<Keypair>cruxManager).publicKey
					: <PublicKey>cruxManager,
	...
	async updateCruxParams(
		...
		console.log(
			"updating crux parameters for crux account with pubkey: ",
			crux.toBase58()
		);

		// Transaction
		const txSig = await this.challengerProgram.methods
			.updateCruxParams(cruxFees)
			.accounts({
				crux: crux,
				cruxManager: isKp(cruxManager)
					? (<Keypair>cruxManager).publicKey
	...
	async payoutFromTreasury(
		...
		console.log(
			"paying out from treasury for crux account with pubkey: ",
			crux.toBase58()
		);

		// Transaction
		const txSig = await this.challengerProgram.methods
			.payoutFromTreasury(cruxTreasuryBump)
			.accounts({
				crux: crux,
				cruxManager: isKp(cruxManager)
					? (<Keypair>cruxManager).publicKey
	...
	async createUserProfile(crux: PublicKey, profileOwner: PublicKey | Keypair) {
		const profileOwnerKey = isKp(profileOwner)
			? (<Keypair>profileOwner).publicKey
			: <PublicKey>profileOwner;

		// Derive PDAs
		const [cruxTreasury, cruxTreasuryBump] = await findCruxTreasuryPDA(crux);
		const [userProfile, userProfileBump] = await findUserProfilePDA(
			crux,
			profileOwnerKey
		);
		...
		console.log(
			"creating user profile account with pubkey: ",
			userProfile.toBase58()
		);

		// Transaction
		const txSig = await this.challengerProgram.methods
			.createUserProfile(cruxTreasuryBump)
			.accounts({
				crux: crux,
				cruxTreasury: cruxTreasury,
				profileOwner: isKp(profileOwner)
					? (<Keypair>profileOwner).publicKey
					: <PublicKey>profileOwner,
	...
	async editUserProfile(
		...
		console.log(
			"editing user profile account with pubkey: ",
			userProfile.toBase58()
		);

		// Transaction
		const txSig = await this.challengerProgram.methods
			.editUserProfile(userProfileBump)
			.accounts({
				crux: crux,
				profileOwner: isKp(profileOwner)
					? (<Keypair>profileOwner).publicKey
	...
	async deleteUserProfile(
		...
		console.log(
			"deleting user profile account with pubkey: ",
			userProfile.toBase58()
		);

		// Transaction
		const txSig = await this.challengerProgram.methods
			.deleteUserProfile(userProfileBump)
			.accounts({
				crux: crux,
				profileOwner: isKp(profileOwner)
					? (<Keypair>profileOwner).publicKey
	...
	async addModerator(userProfile: PublicKey, cruxManager: PublicKey | Keypair) {
		...
		// Derive PDAs
		const [userProfileKey, userProfileBump] = await findUserProfilePDA(
			crux,
			profileOwner
		);
		...
		console.log(
			"adding moderator status to user profile account with pubkey: ",
			userProfile.toBase58()
		);

		// Transaction
		const txSig = await this.challengerProgram.methods
			.addModerator(userProfileBump)
			.accounts({
				crux: crux,
				cruxManager: isKp(cruxManager)
					? (<Keypair>cruxManager).publicKey
	...
	async removeModerator(
		...
		console.log(
			"removing moderator status from user profile account with pubkey: ",
			userProfile.toBase58()
		);

		// Transaction
		const txSig = await this.challengerProgram.methods
			.removeModerator(userProfileBump)
			.accounts({
				crux: crux,
				cruxManager: isKp(cruxManager)
					? (<Keypair>cruxManager).publicKey
	...
	async createChallenge(
		...
		const moderatorKey = isKp(moderator)
			? (<Keypair>moderator).publicKey
			: <PublicKey>moderator;

		// Derive PDAs
		const [moderatorProfile, moderatorProfileBump] = await findUserProfilePDA(
			crux,
			moderatorKey
		);
		const [challenge, challengeBump] = await findChallengePDA(
			crux,
			challengeSeed
		);
	...
	async deleteChallenge(
		...
		// Transaction
		const txSig = await this.challengerProgram.methods
			.deleteChallenge(moderatorProfileBump, challengeBump)
			.accounts({
				crux: cruxKey,
				moderator: isKp(moderator)
					? (<Keypair>moderator).publicKey
	...
	async createSubmission(
		...
		const [submission, submissionBump] = await findSubmissionPDA(
			challenge,
			userProfile
		);
		...
		// Transaction
		const txSig = await this.challengerProgram.methods
			.createSubmission(cruxTreasuryBump, userProfileBump, challengeBump)
			.accounts({
				crux: cruxKey,
				cruxTreasury: cruxTreasury,
				profileOwner: isKp(profileOwner)
	...
	async editSubmission(
		...
		// Transaction
		const txSig = await this.challengerProgram.methods
			.editSubmission(userProfileBump, challengeBump, submissionBump)
			.accounts({
				crux: cruxKey,
				profileOwner: isKp(profileOwner)
					? (<Keypair>profileOwner).publicKey
	...
	async deleteSubmission(
		...
		// Transaction
		const txSig = await this.challengerProgram.methods
			.deleteSubmission(userProfileBump, challengeBump, submissionBump)
			.accounts({
				crux: cruxKey,
				profileOwner: isKp(profileOwner)
					? (<Keypair>profileOwner).publicKey
	...
	async deleteSubmissionModerator(
		...
		console.log(
			"moderator deleting submission with pubkey: ",
			submission.toBase58()
		);
	...
	async closeAccount(signer: PublicKey | Keypair, accountToClose: PublicKey) {
		// Create Signers Array
		const signers = [];
